...
const Product = ({ product, error }) => {
  ...
  <>
    ...
    <div className="lg:w-[30%] flex-1 py-3 px-1">
      ...
      <div onClick={()=>setDelete(true)} className="h-10 w-10 flex justify-center items-center shadow-md rounded-full cursor-pointer">
    ...
    />
      ...
      {
    Delete &&(
          <DeleteConfirmationModal productName={product?.productName} setDelete={setDelete} productID={product?._id} />
    )
      }
  </>
  );
...
const DeleteConfirmationModal = ({productName,setDelete,productID})=>{
  ...
  <div className="fixed flex justify-center items-center bg-[#212a2f]/30 top-0 h-screen w-screen z-50">
    ...
    <p className="font-semibold text-lg tracking-wide">
          Sure! You Want to Delete  <span className="font-bold text-xl text-red-400 underline underline-offset-4"><q> {productName} </q></span>  Product
    </p>
    <div className="mt-4 flex justify-evenly p-2 items-center w-[40%] ml-auto">
          <button onClick={deleteRequest} className="bg-red-400 text-white font-semibold text-lg py-1 px-4 rounded-md shadow-md ">
      Yes
          </button>
          <button onClick={()=>setDelete(false)} className="font-semibold text-lg py-1 px-4 rounded-md shadow-md">
      No
          </button>
    </div>
      </div>
